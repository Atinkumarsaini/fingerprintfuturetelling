...
def poincare_index_at(i, j, angles, tolerance):
    ...
    if 180 - tolerance <= index <= 180 + tolerance:
        return "delta"  # Changed from "loop" to "delta"
    if -180 - tolerance <= index <= -180 + tolerance:
        return "loop"  # Changed from "delta" to "loop"
    if 360 - tolerance <= index <= 360 + tolerance:
        return "whorl"
    return "none"
...
def find_farthest_points(delta_points, loop_points, W):  # Changed parameter order
    if not delta_points or not loop_points:  # Changed condition
        return None, None
    
    max_distance = -float('inf')
    farthest_delta = None  # Changed from farthest_loop to farthest_delta
    farthest_loop = None  # Changed from farthest_delta to farthest_loop
    
    for delta_point in delta_points:  # Changed loop variable
        delta_center = get_box_center(delta_point, W)
        for loop_point in loop_points:  # Changed loop variable
            loop_center = get_box_center(loop_point, W)
            distance = np.sqrt((delta_center[0] - loop_center[0])**2 + 
                             (delta_center[1] - loop_center[1])**2)
            if distance > max_distance:
                max_distance = distance
                farthest_delta = delta_center  # Changed from farthest_loop to farthest_delta
                farthest_loop = loop_center  # Changed from farthest_delta to farthest_loop
    
    return farthest_loop, farthest_delta  # Changed return order
...
def calculate_singularities(im, angles, tolerance, W, mask):
    ...
    singularity_points = {
        "delta": [],  # Changed from "loop" to "delta"
        "loop": [],   # Changed from "delta" to "loop"
        "whorl": []
    }
    
    colors = {
        "delta": (0, 128, 255),  # Changed from "loop" to "delta"
        "loop": (0, 0, 255),      # Changed from "delta" to "loop"
        "whorl": (255, 153, 255)
    }
    ...
    # Process ridge counting
    delta_count = len(singularity_points["delta"])  # Changed from loop_count to delta_count
    loop_count = len(singularity_points["loop"])    # Changed from delta_count to loop_count
    
    delta_center, loop_center = find_farthest_points(  # Changed parameter order
        singularity_points["delta"], 
        singularity_points["loop"],
        W
    )
    ...
    ridge_count = 0
    if delta_center and loop_center:  # Changed condition
        # Draw thin red line
        cv.line(result, delta_center, loop_center, (0, 0, 255), 1, cv.LINE_AA)  # Changed parameter order
        ...
        # Find and mark ridge intersections
        intersections = find_ridge_intersections(binary, delta_center, loop_center)  # Changed parameter order
        ridge_count = len(intersections)
    ...
    return result, ridge_count, loop_count, delta_count  # Return values remain unchanged